...
export const BigBase64 = {
  _Rixits: process.env.RIXITS!,

  fromNumber: function (number: BigNumber) {
    if (number.isNaN() || !number.isInteger())
      throw `The input: ${number} is not valid`;
    if (number.isNegative()) throw "Can't represent negative numbers now";
...
function generateBinaryString(min: number, max: number): string {
...
async function generateRandomEncode(char: string) {
  let code = generateBinaryString(
    +process.env.CODE_MIN!,
    +process.env.CODE_MAX!
  );
  let encode;
...
export async function encrypt(message: string, key?: string): Promise<string> {
  ...
  if (key) {
    ...
    encryptedMessage += key.length
      .toString(2)
      .padStart(+process.env.KEY_PAD!, "0");

    for (let x of key) {
      let randomizer = generateBinaryString(
        +process.env.ENCRYPTION_MASK_MIN!,
        +process.env.ENCRYPTION_MASK_MAX!
      );
      ...
      encryptedMessage +=
        randomizer.length.toString(2) +
        zerOne() +
        encodedChar.length.toString(2).padStart(+process.env.PAD!, "0") +
        zerOne() +
        randomizer +
        encodedChar +
        zerOne();
...
export async function decrypt(
  ...
  if (keyStatus === process.env.KEY_ON!) {
    const keyLen = parseInt(
      encryptedMessage.substring(0, +process.env.KEY_PAD!),
      2
    );
    ...
    for (let i = 0; i < keyLen; i++) {
      let randomizer = parseInt(
        encryptedMessage.substring(0, +process.env.RAND_END!),
        2
      );
      let charLength = parseInt(
        encryptedMessage.substring(
          +process.env.LEN_START!,
          +process.env.LEN_END!
        ),
        2
      );
